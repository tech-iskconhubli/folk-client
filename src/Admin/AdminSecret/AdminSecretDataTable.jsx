...
const AdminSecretDataTable = () => {
  ...
  const deleteHandler = (id) => {
    // console.log(id)
     dispatch(deleteAdminSecretFormData(id))
     .then(res=>{
         console.log(res)
                  ...
                  <Tbody fontSize={"14px"} fontWeight={"400"}>
                      {store?.map((item, index) => (
                          <Tr key={item.id}>
                              ...
                              <Link to={`/admin/secret/data/edit/${item._id}`}><Td><Button fontSize={"14px"} fontWeight={"400"} color={"blue"}>Edit</Button></Td></Link>
...
export default AdminSecretDataTable