...
import { singleAdminBlogsFormData, updateAdminBlogsFormData } from '../../Redux/app/action';
import AdminTopNavbar from '../../Components/AdminNavbar/AdminTopNavbar';

const AdminBlogsDataEdit = () => {
  const init ={
        date: "",
        title: "",
        description: "",
  ...
  useEffect(()=>{
    dispatch(singleAdminBlogsFormData(id))
    .then(res=>{
  ...
  dispatch(updateAdminBlogsFormData(id,formData))
  .then(res=>{
      console.log("res", res);
                  ...
                  <form onSubmit={handleSubmit} >
                   <Stack gap={"20px"} >
                        <Box   display={"flex"} justifyContent={"center"} alignItems={"center"} gap={"0px"}>
                              ...
                              <Box width={"90%"}>  <Input borderColor={"#2B3553"} type='text' name={"date"} value={formData?.date } onChange={handleChange}   placeholder='please enter Residency name'/></Box>
                        </Box>


                        <Box  display={"flex"} justifyContent={"center"} alignItems={"center"} gap={"0px"}>
                              <Box width={"20%"}> <FormLabel  fontSize={"14px"} fontWeight={"500"} fontFamily={"body"}>Enter title</FormLabel></Box>
                              <Box width={"90%"}>  <Input borderColor={"#2B3553"} type='text' name={"title"} value={formData?.title} onChange={handleChange}  placeholder='please enter Location'/></Box>
                        </Box>
...
export default AdminBlogsDataEdit